...
function AppShell() {
  ...
  const handleZenTap = async () => {
    setZenState("loading");
    try { const data = await apiRequest<{ zenHits?: number }>("/api/modes/zen", { method: "POST" }); setCounts((c) => ({ ...c, zenHits: data.zenHits ?? c.zenHits })); setZenState("success"); addBurst(language === "zh" ? "ÌÛÍ´ -1" : "Pain -1"); } catch { setZenState("error"); }
  };
      ...
      <header className="top-nav">
        ...
        <div className="top-actions">
          ...
          <div className="language-switch" role="group" aria-label={t.language}>
            ...
            <button className={`language-switch__item ${language === "zh" ? "is-active" : ""}`} type="button" onClick={() => setLanguage("zh")}>ÖÐÎÄ</button>
          </div>
